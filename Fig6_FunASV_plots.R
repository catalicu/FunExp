...
#Functional data is not necessary here explicitly, but it means we are using the same processed tables to rank and compare taxa. 

# start from the sorted table of slopes
ASVslope_sorted=read.table('output_files/ASVslope_sorted_FunExp12023-01-07.txt', header=TRUE)
...
### Combine these values into useable tables
...
# Save this table:
ASVrel.abund.pa_printPATH = paste(outPATH, 'FunExp_Relabund_freqRanking_perASV', date,'.txt', sep='')
write.table(ASVrel.abund.pa, file=ASVrel.abund.pa_printPATH, sep='\t')
...
#save this table: major output
ASVrel.pa.fun_table = paste(outPATH, 'ASV_relabun_pa_fun_table_FunExp1', date,'.txt', sep='')
write.table(ASVrel.abund.pa.fun, ASVrel.pa.fun_table, sep="\t")
#Add the information on taxon classification according to their relationship with function (from FunASV_v8_taxAnalysis_abund-funct_B.R)
...
ggplot(ASVrel.abund.pa.fun.sorted, aes(log(ASVrel.abund), ASVpa.freq)) + geom_point(aes(fill=Slope), size=3, shape=21) + Theme +ggtitle('Relationship between ASV occupancy (frequency) and relative abundance') + scale_fill_manual(values=c('black', 'grey', 'white', 'blue')) + ylab('ASV occurrence') + xlab('ASV Relative Abundance')
...
otumelt_fun.div=melt(ASVfor_melt, id=c("leaf_age_weeks2"))
